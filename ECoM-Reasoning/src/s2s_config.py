...
class VocabConfig:
    ...
    ###### com token ######
    com_qt: int = field(init=False) # start of question text
    com_at: int = field(init=False) # start of answer text
    com_think: int = field(init=False) # start of think text
    end_think: int = field(init=False) # end of think text
    com_aat: int = field(init=False)# start of answer audio (but it's text token)
    ...
    def __post_init__(self):
        ...
        self.com_qt = self.text_vocabsize + 5 #
        self.com_at = self.text_vocabsize + 6 #
        self.com_aat= self.text_vocabsize + 7 #
        self.com_think = self.text_vocabsize + 8 #
        self.end_think = self.text_vocabsize + 9 #
...
@dataclass
class ModelConfig:
    file: str = "src/model/slam_model_s2t.py:model_factory"
...
@dataclass
class DataConfig:
    dataset: str = "speech_dataset_s2t"
    file: str = "src/speech_dataset_s2t.py:get_speech_dataset"
    dataset_version: str = "s2s"
    train_data_path: Optional[str] = None
    val_data_path: Optional[str] = None
    # compress_rate: float = 1.0
    # compress_stage: Optional[str] = None
...
class DecodeConfig:
    ...
    do_layershift: bool = True
    num_latency_tokens: int = 0
    hidden_dump_dir: Optional[str] = None
    hidden_dump_tag: Optional[str] = None
    hidden_dump_dtype: str = "float16"
...
class LogConfig:
    ...
    wandb_dir: str = "./exp/wandb_log"
    wandb_entity_name: str = "project_name"
    wandb_project_name: str = "project_name"
    wandb_exp_name: str = "exp_name"
    log_file: str = "./exp/log/test.log"
    log_interval: int = 10
