...
def generate_from_wav(wav_path, model, codec_decoder, dataset_config, decode_config, logger, device, model_config, tone_dir, audio_prompt_path=None, output_text_only=False, layer_shift=layershift, inference_streaming=False):
	...
	if "paral" in dataset_config.dataset_version: # parallel (multi-output channels)
	...
	else: # serial or interleaved (single-output channel)
		prompt_ids = torch.tensor(prompt_ids).unsqueeze(0) 

		example_ids = [layershift(vocab_config.input_a, 0)] + [layershift(vocab_config.pad_a, 0)] * audio_length + [(layershift(vocab_config.eoa, 0)), special_token_t]
	...
	# print("============== output ==============")
	# print(f"model_outputs shape: {len(model_outputs)}")
	# print(f"text_outputs shape:  {len(text_outputs)}")
	# print(f"audio_outputs shape: {len(audio_outputs[0])}")
