...
def main(kwargs: DictConfig):
    ...
    pred_path = os.path.join(decode_log_dir, "pred_text.jsonl")
    gt_path = os.path.join(decode_log_dir, "gt_text.jsonl")
    question_path = os.path.join(decode_log_dir, "question_text.jsonl")
    ...
    if not os.path.exists(tone_audio_dir) and not (
        output_text_only or decode_config.decode_text_only
    ):
    ...
    logger.info(
        "============== Start {task_type} Inference ==============".format(
            task_type=task_type
        )
    )

    with jsonlines.open(pred_path, mode="w") as pred, jsonlines.open(
        gt_path, mode="w"
    ) as gt, jsonlines.open(question_path, mode="w") as q:
        for step, batch in enumerate(test_dataloader):
            ...
            for key, source_text, target_text, generated_text in zip(
                batch["keys"],
                batch["source_texts"],
                batch["target_texts"],
                [output_text],
                ...
                if target_text is not None:
                    ...
                    gt.write({str(step).zfill(4): source_text})
            ...
            if (
                audio_outputs[0].shape[0] == decode_config.max_new_tokens
            ):  # if the audio token is too long, skip (bad case)
                logger.warning(
                    f"Audio token is too long, skip. You can try to increase the max_new_tokens in the decode_config."
                )
                continue
            ...
            for i, key in enumerate(batch["keys"]):
                ...
                if code_type == "SNAC":
                    ...
                    try:
                        ...
                        logger.warning(f"Something wrong with cosyvoice, skip {i}.")
                        continue
                    if audio_hat is None:
                        logger.warning(f"Something wrong with audio_hat, skip {i}.")
                        continue
                else:
                ...
                sf.write(
                    f"{tone_audio_dir}/{str(step).zfill(4)}.wav",
                    audio_hat.squeeze().cpu().numpy(),
                    speech_sample_rate,
                )
                logger.info(
                    f"Generated Audio: {tone_dir}/{str(step).zfill(4)}.wav, audio length: {audio_length:.2f}s, generation time: {end_time - start_time:.2f}s, RTF: {RTF:.2f}"
                )
