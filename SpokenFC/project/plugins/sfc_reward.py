...
# Assume orms, ORM, and logger are already defined
# from swift.llm import orms, ORM
# from swift.utils import get_logger
# logger = get_logger()


def _serialize_reward_value(value: Any) -> str:
    """Serialize argument values deterministically for edit-distance scoring."""
    if isinstance(value, str):
        return value

    def _normalize_json_value(item: Any) -> Any:
        if isinstance(item, dict):
            return {
                str(key): _normalize_json_value(nested_value)
                for key, nested_value in sorted(item.items(), key=lambda pair: str(pair[0]))
            }
        if isinstance(item, (list, tuple)):
            return [_normalize_json_value(nested_value) for nested_value in item]
        if isinstance(item, (set, frozenset)):
            normalized_items = [_normalize_json_value(nested_value) for nested_value in item]
            return sorted(
                normalized_items,
                key=lambda nested_value: json.dumps(
                    nested_value, ensure_ascii=False, sort_keys=True, separators=(',', ':')))
...
def _normalized_edit_distance(predicted_value: Any, ground_truth_value: Any) -> float:
    """Return Levenshtein distance normalized by the longer serialized value."""
    predicted = _serialize_reward_value(predicted_value)
    ground_truth = _serialize_reward_value(ground_truth_value)
    max_length = max(len(predicted), len(ground_truth), 1)

    if predicted == ground_truth:
        return 0.0
    if not predicted:
        return len(ground_truth) / max_length
    if not ground_truth:
        return len(predicted) / max_length

    if len(predicted) < len(ground_truth):
        predicted, ground_truth = ground_truth, predicted

    previous_row = list(range(len(ground_truth) + 1))
    for row_index, predicted_char in enumerate(predicted, start=1):
        current_row = [row_index]
        for column_index, ground_truth_char in enumerate(ground_truth, start=1):
            insertion_cost = current_row[column_index - 1] + 1
            deletion_cost = previous_row[column_index] + 1
            substitution_cost = previous_row[column_index - 1] + (predicted_char != ground_truth_char)
            current_row.append(min(insertion_cost, deletion_cost, substitution_cost))
        previous_row = current_row

    return previous_row[-1] / max_length


class SpokenFunctionCallReward_V1(ORM):
    """
    Custom reward for evaluating model-generated function-call strings.
    """

    @staticmethod
    def _parse_function_call(call_string: str) -> Tuple[str | None, Dict[str, Any]]:
        """
        Safely parse a function-call-like string.
        (unchanged)
        """
    ...
    def __call__(self, completions: List[str], **kwargs) -> List[float]:
        """
        Called by the Swift framework to compute rewards.

        Args:
            completions (List[str]): Batch of model outputs.
            **kwargs: Other dataset columns in batched form.
                      Ground truth is read directly from kwargs['solution'].

        Returns:
            List[float]: Reward score for each sample.
        """
        rewards = []
        
        # Read batched solution data from kwargs
        ...
        for i, response in enumerate(completions):
            # Get ground truth for the current sample by index
            ground_truth = batched_solutions[i]

            # Assign minimum reward when ground truth is missing
            if not ground_truth:
                rewards.append(-1.0)
                continue
            
            # Log model output and ground truth for debugging
            logger.info(f"[Sample {i}] Model Response: '{response}'")
            logger.info(f"[Sample {i}] Ground Truth:   '{ground_truth}'")

            # Step 1: Parse model output and ground truth
            ...
            if not gt_keys:
                ...
                value_similarity = sum(
                    1.0 - _normalized_edit_distance(gen_params[key], gt_params[key])
                    for key in common_keys)
                r_values = value_similarity / len(gt_keys)

            # Step 3: Compute weighted final reward
...
# Register custom reward with the Swift ORM registry
...
class SpokenFunctionCallReward_V2(ORM):
    """
    Custom reward for evaluating model-generated function-call string lists.
    
    Supports stringified lists (e.g. "[\"func1()\", \"func2()\"]")
    compared against a similar ground-truth list with an F1-like aggregate reward.
    """
    ...
    def _parse_function_call(call_string: str) -> Tuple[str | None, Dict[str, Any]]:
        ...
        try:
            # Build a parseable expression via f-string
            tree = ast.parse(f"dummy_func({args_str})", mode='eval')
            call_node = tree.body
            ...
            if isinstance(call_node, ast.Call):
                ...
                for keyword in call_node.keywords:
                    try:
                        ...
                        # (ast.literal_eval handles AST literal nodes)
                        kwargs[keyword.arg] = ast.literal_eval(keyword.value)
                    except ValueError:
                        # Skip non-literal values (e.g. variable names)
                        pass
                return func_name, kwargs

            # If parse result is not a call expression
            return func_name, {}
        except Exception as e:
            # Catch parse errors (e.g. syntax errors including leading zeros)
            # Can happen when args_str is malformed (e.g. key=09:00)
    ...
    def _parse_call_list(self, call_list_string: str) -> List[str]:
        """
        Safely parse a string representation of a list into string items.
        
        Strategy:
        1. Try ast.literal_eval for strict parsing (well-formed strings).
        2. On failure (e.g. SyntaxError from unescaped quotes),
           fall back to regex to find all func(...) patterns.
        """
        ...
        # Strategy 1: strict parse (escaped strings from model output)
        try:
            ...
            if isinstance(parsed_list, list) and all(isinstance(item, str) for item in parsed_list):
                return parsed_list
                 ...
                 # Handle model returning a single string instead of a list
                 # Check whether it looks like a function call
                 if re.match(r'\w+\(.*\)', parsed_list, re.DOTALL):
                     return [parsed_list]
                 
        except (ValueError, SyntaxError, TypeError) as e:
            # Strategy 2: regex fallback (malformed dataset strings)
            # This SyntaxError matches the leading zeros error in logs
            logger.warning(f"ast.literal_eval failed: '{e}'. Falling back to regex parser for string: '{call_list_string}'")
            
            try:
                # Regex finds all func_name(...) patterns,
                # handles one level of nested (), ignores [] and {}.
                # Usually robust enough for malformed strings.
                # \w+\(           - func(
                ...
                if calls:
                    ...
                    # If regex fails but the string looks like a function call
                    # (e.g. unbalanced parentheses caused regex failure)
                    # Make a final fallback guess
        ...
        # If literal_eval returned non-list/str or regex failed
    ...
    def _calculate_single_call_reward(self, gen_call: str, gt_call: str) -> float:
        """
        Compute reward between one generated call and one ground-truth call.
        Reward is in [-1.0, 1.0].
        (unchanged)
        """
        # Step 1: Parse generated and ground-truth calls
        gen_name, gen_params = self._parse_function_call(gen_call)
        gt_name, gt_params = self._parse_function_call(gt_call)

        # Step 1.5: Handle parse failures
        
        # If ground truth cannot be parsed (empty or invalid data)
        if not gt_name:
            # Treat as correct if the model output is also unparseable
            # Treat as wrong if model is valid but GT is invalid
            return 1.0 if not gen_name else -1.0
        
        # If GT is valid but model output is invalid
        if not gen_name:
            return -1.0

        # Step 2: Compute component scores (gen_name and gt_name are valid)
        
        # 2.1: Function name score
        r_name = 1.0 if gen_name == gt_name else 0.0

        # 2.2: Argument key score (Jaccard similarity)
        gen_keys = set(gen_params.keys())
        gt_keys = set(gt_params.keys())
        
        if not gt_keys:
            # If GT needs no args, the model should not provide any
            r_keys = 1.0 if not gen_keys else 0.0
        else:
            intersection_keys = len(gen_keys.intersection(gt_keys))
            union_keys = len(gen_keys.union(gt_keys))
            # Jaccard similarity
            r_keys = intersection_keys / union_keys if union_keys > 0 else 1.0 # (union=0 means both empty; Jaccard=1)

        # 2.3: Argument value score (over shared keys)
        common_keys = gen_keys.intersection(gt_keys)
        ...
        # Step 3: Compute weighted final reward (normalized to [-1.0, 1.0])
    ...
    def __call__(self, completions: List[str], **kwargs) -> List[float]:
            """
            Called by the Swift framework to compute rewards.
            (modified to support multi-turn data structures)
            """
            rewards = []
            
            # Read batched solution data from kwargs
            # In multi-turn data, 'solution' is often a list of dicts from JSON
            batched_solutions = kwargs.get('solution')
            if not batched_solutions or len(completions) != len(batched_solutions):
            ...
            # Base reward for empty matches
            EMPTY_MATCH_REWARD = -1.0 # (r_name=0, r_keys=0, r_values=0) -> (0-1.5)*2/3 = -1.0

            # Iterate over each sample in the batch
            for i, response_str in enumerate(completions):
                # response_str (completions[i]) is the model's final-turn response
                
                # =====================  CORE CHANGES HERE  ===================== #
                
                # Get ground-truth payload for the current sample
                sample_data = batched_solutions[i]
                gt_str = None

                # Extract the 'solution' field from the payload dict
                if isinstance(sample_data, dict):
                    gt_str = sample_data.get('solution')
                elif isinstance(sample_data, str):
                    # Fallback when 'solution' is already a string/list
                    gt_str = sample_data
                
                # ================================================================= #

                # Missing ground truth when gt_str is None or empty
                if not gt_str:
                ...
                # response_str is the model's final-turn output
                logger.info(f"[Sample {i}] Model Response: '{response_str}'")
                # gt_str is the content of the 'solution' field
                logger.info(f"[Sample {i}] Ground Truth:   '{gt_str}'")

                # Step 1: Parse strings into call lists via robust parser
                ...
                # Step 2: F1-like reward (average of precision and recall)
                ...
                if not gen_list:
                    ...
                    for g_call in gen_list:
                        best_r_for_g = max(
                            ...
                            or [EMPTY_MATCH_REWARD] # when gt_list is empty
                        )
                ...
                if not gt_list:
                    ...
                    for t_call in gt_list:
                        best_r_for_t = max(
                            ...
                            or [EMPTY_MATCH_REWARD] # when gen_list is empty
                        )
...
# Assume ORM, orms, and logger are defined elsewhere per Swift
...
class SpokenFunctionCallReward_V3(ORM):
    ...
    [Modified] First extract content from <answer>...</answer> tags,
    then parse function calls and compute F1 reward on the extracted text.
    """
    ...
    def __call__(self, completions: List[str], **kwargs) -> List[float]:
        """
        Called by the Swift framework to compute rewards.
        [Modified] Extract content from <answer> tags first.
        """
        rewards = []
        ...
        for i, response_str_raw in enumerate(completions):
            # Raw ground-truth string for the current sample
            ...
            # ========================================================= #
            # Step 0: Extract content from <answer> tags
            # ========================================================= #
            
            # 1. Extract ground truth (re.DOTALL for multiline)
            ...
            # 2. Extract model output (re.DOTALL for multiline)
            ...
            # Step 1: Parse extracted strings into call lists
            gen_list = self._parse_call_list(response_str)
            gt_list = self._parse_call_list(gt_str)
            ...
            # 2.1: Precision
            if not gen_list:
                # If the model generated no calls
                # P=1.0 when GT needs no calls or there are no wrong calls
                # 1.0 is consistent with the original logic here
                avg_precision = 1.0
            else:
                precision_rewards = []
                for g_call in gen_list:
            ...
            # 2.2: Recall
            if not gt_list:
                # If GT requires no calls
                # R=1.0 when nothing is required or generated
                avg_recall = 1.0
            else:
                recall_rewards = []
                for t_call in gt_list:
...
# Register custom reward with the Swift ORM registry
# Registered under a new name to distinguish from the original
