...
class SpokenFunctionCallReward(ORM):
    """
...
orms['sfc_reward'] = SpokenFunctionCallReward
...
class SpokenFunctionCallReward(ORM):
    ...
    def __call__(self, completions: List[str], **kwargs) -> List[float]:
        ...
        for i, response_str in enumerate(completions):
            # 获取当前样本的标准答案 (ground_truth) 字符串
            gt_str = batched_solutions[i]
...
# 向Swift框架的ORM注册表中注册我们自定义的奖励函数
orms['sfc_reward_list'] = SpokenFunctionCallReward